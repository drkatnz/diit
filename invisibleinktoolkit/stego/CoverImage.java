...
public class CoverImage{
	...
	throws IllegalArgumentException{
		...
		//check the bit range
		if((mCover.getType() == BufferedImage.TYPE_USHORT_555_RGB
				|| mCover.getType() == BufferedImage.TYPE_USHORT_565_RGB )
				&& bitpos > 4)
			throw new IllegalArgumentException
			("Bit position in incorrect position for image (0-4)!");
		else if (bitpos > 7 || bitpos < 0)
			throw new IllegalArgumentException
			("Bit position in incorrect position for image (0-7)!");
