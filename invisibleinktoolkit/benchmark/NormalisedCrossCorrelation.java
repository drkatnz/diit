...
/**
 * A measure of normalised cross correlation.
 *
 *
 * @author Kathryn Hempstalk.
 */
public class NormalisedCrossCorrelation 
extends PixelBenchmark 
implements Benchmark{
	
	
	/**
	 * Calculates the normalised cross-correlation of two images.
	 * The formula used is:
	 * <BR>
	...
	public double calculate(BufferedImage original,
		...
		for(int i = 0; i < original.getHeight(); i++){
			for(int j = 0; j < original.getWidth(); j++){
				...
				originaldiff += Math.pow(getRed(origpix), 2) + 
						Math.pow(getGreen(origpix), 2) +
						Math.pow(getBlue(origpix), 2);
