 ...
 * their neighbours.
 * <BR>
 * This class represents a traditional laplace graph, simply
 * meaning it works only on black and white values in an image, and
 * does not produce absolute values.
 *
...
public class TraditionalLaplace implements Filter{
	 ...
	 *
	 */
	public TraditionalLaplace(int startbits, int endbits){
	 ...
	 *
	 */
	public TraditionalLaplace(BufferedImage image, int startbits, int endbits){
	...
	public int getValue(int xpos, int ypos) throws Exception{
		...
		int value = (pixcount * this.getLuminance(pixelval)) -
		(this.getLuminance(leftpix) + this.getLuminance(rightpix)
				+ this.getLuminance(uppix) + this.getLuminance(downpix));
	...
	/**
	 * Gets the grayscale colour value (luminance) of a pixel.
	 * 
	 * @param pixel The pixel to get the value of.
	 * @return The luminance value.
	 */
	protected int getLuminance(int pixel){
		//Y = 0.299R + 0.587G + 0.114B
		return (int)((0.299 * this.getRed(pixel)) + (0.587 * this.getGreen(pixel))
			+ (0.114 * this.getBlue(pixel)));
	}
