 ...
 *
 * A prewitt edge detector is very similar to a sobel filter.  It 
 * detects the edges in a picture by using a pair of convolution
 * masks and calculating out the magnitude of the gradients of
 * these masks.  
 * <P>
 * The only difference between the prewitt and sobel edge detectors
 * is the convolution masks used.
 *
...
public class Prewitt implements Filter{
	...
	//CONSTRUCTORS
	/**
	 * Creates a new prewitt edge filter, with default
	...
	/**
	 * Creates a new prewitt edge filter, with no image, but
	 ...
	 *
	 */
	public Prewitt(int startbits, int endbits){
		this(null, startbits, endbits);
	}
	
	/**
	 * Creates a new prewitt edge filter with given image and
	 ...
	 *
	 */
	public Prewitt(BufferedImage image, int startbits, int endbits){
	...
	/**
	 * Produces a prewitt filter value on a given pixel.
	 * <P>
	 * A prewitt filter uses a pair of convolution masks on the image
